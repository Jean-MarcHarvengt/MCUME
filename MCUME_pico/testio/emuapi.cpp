...
static int readNbFiles(char * rootdir) {
  ...
  f_closedir(&dir);
#else
  strncpy(&files[totalFiles++][0], "filename1", MAX_FILENAME_SIZE-1);  
  strncpy(&files[totalFiles++][0], "filename2", MAX_FILENAME_SIZE-1);  
  strncpy(&files[totalFiles++][0], "filename3", MAX_FILENAME_SIZE-1);  
  strncpy(&files[totalFiles++][0], "filename4", MAX_FILENAME_SIZE-1);  
  strncpy(&files[totalFiles++][0], "filename5", MAX_FILENAME_SIZE-1);  
#endif
